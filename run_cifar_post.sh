#!/bin/bash
# run_cifar_post.sh — Scattering-init post-experiments (70 per dataset)
# Usage: ./run_cifar_post.sh <num_gpus> <dataset>
...
COMMON="--dataset $DATASET --lr-epochs 20 --epochs 150 --global-batch-size 128 --print-freq 200 --workers 10 --kernel-size 5"
BASE_DIR="cifar_post_${DATASET}"
LOG_DIR="logs_post_scat_${DATASET}"
mkdir -p "$LOG_DIR"
...
for TS in 0.05 0.1 0.15 0.2 0.3 0.5 1.0; do
  ...
  for SEED in 42 43 44 45 46; do
    SAVE="${BASE_DIR}/3b_cmod_4L_s${TS_TAG}_seed${SEED}"
    if [ "$TS" = "1.0" ]; then
      TS_ARG=""
    else
...
for TS in 0.05 0.1 0.15 0.2 0.3 0.5 1.0; do
  TS_TAG=$(echo "$TS" | sed 's/0\.//;s/1\.0/10/')
  for SEED in 42 43 44 45 46; do
    SAVE="${BASE_DIR}/4b_cmod_4L_h27_s${TS_TAG}_seed${SEED}"
    if [ "$TS" = "1.0" ]; then
      TS_ARG=""
    else
      TS_ARG="--train-size $TS"
...
echo "Scattering-init post ($DATASET): $TOTAL experiments, $NGPUS GPUs, $NBATCHES batches"
echo ""
