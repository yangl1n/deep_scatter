#!/bin/bash
# run_cifar_rand.sh — Random-init experiments (72 total)
# Usage: ./run_cifar_rand.sh <num_gpus>
set -euo pipefail
...
COMMON="--random-init --joint --epochs 120 --global-batch-size 128 --print-freq 200 --workers 10 --kernel-size 5"
BASE_DIR="cifar_exps"
LOG_DIR="logs_rand"
mkdir -p "$LOG_DIR"
    ...
    for L in 4 6 8; do
      ...
      for H in "${HORIZONS[@]}"; do
        ...
        for TS in 0.1 0.3 0.5 1.0; do
          ...
          SAVE="${BASE_DIR}/${ARCH_TAG}_rand_s${TS_TAG}"
          if [ "$TS" = "1.0" ]; then
            TS_ARG=""
          else
            TS_ARG="--train-size $TS"
          fi
          JOBS+=("$ARCH_ARGS $TS_ARG --save-dir $SAVE")
        done
      done
    done
  done
...
echo "Random-init: $TOTAL experiments, $NGPUS GPUs, $NBATCHES batches"
echo ""
